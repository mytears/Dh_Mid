let m_this_name = "situation";
let m_contents_url = "";
let m_notice_mode = "";
let m_root_url = "";
let m_situation_list = [];
...
function setInitSetting() {
    $("#id_img_0").attr("src",convFilePath(m_situation_list.school_file_path));
    $("#id_img_1").attr("src",convFilePath(m_situation_list.facility_file_path));
    $(".img_zone img").hide();
...
function setContents() {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            m_situation_list = data.situation_list;
            setInitSetting();
        },
        error: function (xhr, status, error) {
            console.error('컨텐츠 에러 발생:', status, error);
...
function setDataInit(_contents, _notice_mode){
    ...
    m_situation_list = m_contents_json.situation_list;
    setInitSetting();
}
